...
rm ~/.config/kitty/AtomOneLight.conf
ln -s "$(pwd)/kitty" ~/.config/kitty/AtomOneLight.conf
...
rm ~/.tmux/status.conf
ln -s "$(pwd)/status.conf" ~/.tmux/status.conf
rm ~/.bash_profile
...
rm -rf ~/.config/.tridactylrc
ln -s "$(pwd)/tridactylrc" ~/.config/.tridactylrc
rm ~/.gdbinit
