...
local function todo(opts)
  ...
  coroutine.wrap(function()
    ...
    for _, buf in pairs(buffers) do
      ...
      local bufnrstr = string.format("%s%s%s", leftbr, utils.ansi_codes.yellow(string.format(buf.bufnr)), rightbr)
      local buficon = ""
    ...
    opts.preview = act
    opts._fzf_cli_args = utils._if(header_line and not opts.ignore_current_buffer, "--header-lines=1", "")

    local selected = require("fzf").fzf(items, core.build_fzf_cli(opts), config.winopts(opts))
